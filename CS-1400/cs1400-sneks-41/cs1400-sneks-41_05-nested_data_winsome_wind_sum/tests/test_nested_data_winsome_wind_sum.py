...
class TestNestedDataWinsomeWindSum(asttest.ASTTest):

    def setUp(self):
        super().setUp("nested_data_winsome_wind_sum.py")
    ...
    def test_required_syntax(self):
        ...
        self.assertNotIn(7, nums, "Are you sure that is the rainfall for only "
                "windy days?")
        self.assertNotIn(5, nums, "Do not embed the solution in your "
        ...
        self.assertGreaterEqual(len(subscripts), 3, "You will need to use at "
                "least three dictionary lookups (but most likely four, "
                "depending on how you do it).")
        self.assertLessEqual(len(subscripts), 4, "You only need to use at most"
                " four dictionary lookups.")

        str_values = [s.s for s in self.find_all(ast.Str, loop)]
        if "weather" in str_values or "rainfall" in str_values or "windy?" in str_values:
        ...
        if "Weather" in names or "Rainfall" in names:
            self.fail("You should use a string literal value to look up the "
                    "name, not a variable. If you are trying to create a "
                    "variable name Weather, I recommend using all lower case "
                    "for your variable names in Python.")
        if "Weather" not in str_values or "Rainfall" not in str_values or "Windy?" not in str_values:
        ...
        self.assertNotEqual(len(self.find_all(ast.If, loop)), 0, "You will "
                "need to use an if statement.")
        self.assertEqual(len(self.find_all(ast.Eq, loop)), 0, "Remember, when "
                "checking if a boolean is True, it is redundant to use "
                "`== True` because it is already True (or False).")
        ...
        self.assertNotEqual(self.printed_lines[0], 7, "Are you sure that is "
                "the rainfall for only the windy days?")
        self.assertEqual(self.printed_lines[0], 5, "You are not printing out "
                "the right value.")
