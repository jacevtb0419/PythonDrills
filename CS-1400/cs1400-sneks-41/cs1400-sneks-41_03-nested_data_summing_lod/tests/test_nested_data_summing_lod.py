...
class TestNestedDataSummingLod(asttest.ASTTest):

    def setUp(self):
        super().setUp("nested_data_summing_lod.py")

    def test_required_syntax(self):
        nums = [n.n for n in self.find_all(ast.Num)]
        self.assertNotIn(73, nums, "Do not embed the solution in your "
        ...
        loop = fors[0]
        if isinstance(loop.target, ast.Name) and loop.target.id.lower() == "score":
        ...
        self.assertNotEqual(len(subscripts), 0, "You will need to use a "
                "dictionary lookup.")
        self.assertEqual(len(subscripts), 1, "You only need to use one "
                "dictionary lookup.")

        str_values = [s.s for s in self.find_all(ast.Str, loop)]
        self.assertIn("Score", str_values, "Remember, capitalization matters "
                "when it comes to keys!")

        names = [n.id for n in self.find_all(ast.Name, loop)]
        if ("score" in names or "Score" in names) and "Score" not in str_values:
            self.fail("You should use a string literal value to look up the "
                    "name, not a variable. While you could assign the result "
                    "to a variable before printing, you don't need to for this"
                    " problem. Just print the lookup expression directly.")
        self.assertIn("Score", str_values, "You are not using the right key.")
        ...
        self.assertEqual(self.printed_lines[0], 73, "You are not printing out "
                "the right value.")
